...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Gestão de presença dinâmica para stands: geofencing, Wi-Fi local, QR Code, escala e roleta justa." },
    ],
  }),
});
    ...
    <main className="min-h-screen bg-background text-foreground">
      <section className="mx-auto flex min-h-screen max-w-5xl flex-col items-center justify-center px-6 py-16 text-center">
        ...
        <div className="mt-16 grid w-full gap-4 sm:grid-cols-3">
          {[
            { t: "Roleta Justa", d: "Distribuição automática de leads conforme escala e presença." },
            { t: "Presença Validada", d: "Geofencing + Wi-Fi + QR Code dinâmico no check-in." },
            { t: "Relatórios Semanais", d: "Arquivamento de 30 dias e envio automático por e-mail." },
          ].map((f) => (
            <div key={f.t} className="rounded-lg border border-border bg-card p-5 text-left">
        ...
        <p className="mt-12 text-xs text-muted-foreground">
          Backend: Lovable Cloud · Auth: e-mail/senha · Relatórios via Resend
        </p>
      </section>
    </main>
  );
